    ...
    def _update_actor(self, batch: KVBatchMeta, metrics: dict) -> KVBatchMeta:
        ...
        if is_distillation_enabled(self.config.get("distillation")):
            ...
            if not response_mask_values.bool().any():
                # Critique-conditioned OPD intentionally excludes successful
                # trajectories. Avoid an optimizer step entirely when the batch
                # contains no failed-trajectory tokens; a numerical zero loss can
                # still update AdamW parameters through momentum or weight decay.
        ...
        distillation_loss_settings = (
            self.distillation_config.distillation_loss.loss_settings
            if is_distillation_enabled(self.config.get("distillation"))
            else None
        )
        distillation_use_topk = bool(
            distillation_loss_settings
            and (distillation_loss_settings.use_topk or distillation_loss_settings.use_full_vocab)
        )
        distillation_use_full_vocab = bool(
            distillation_loss_settings and distillation_loss_settings.use_full_vocab
        )
        extra_info = {
